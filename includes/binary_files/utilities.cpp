#include "utilities.h"

bool file_exists(const char filename[]){
    const bool debug = false;
    std::fstream ff;
    ff.open (filename,
        std::fstream::in | std::fstream::binary );
    if (ff.fail()){
        if (debug) std::cout<<"file_exists(): File does NOT exist: "<<filename<<std::endl;
        return false;
    }
    if (debug) std::cout<<"file_exists(): File DOES exist: "<<filename<<std::endl;
    ff.close();
    return true;
}

void open_fileRW(std::fstream& f, const char filename[]) throw(const Error_msg){
    const bool debug = false;
    //openning a nonexistent file for in|out|app causes a fail()
    //  so, if the file does not exist, create it by openning it for
    //  output:
    if (!file_exists(filename)){
        //create the file
        f.open(filename, std::fstream::out|std::fstream::binary);
        if (f.fail()){
            if(debug)
                std::cout<<"file open (RW) failed: with out|"<<filename<<"]"<<std::endl;
            throw(Error_msg("file RW failed"));
        }
        else{
            if (debug) std::cout<<"open_fileRW: file created successfully: "<<filename<<std::endl;
        }
    }
    else{
        f.open (filename,
            std::fstream::in | std::fstream::out| std::fstream::binary );
        if (f.fail()){
            if(debug)
                std::cout<<"file open (RW) failed. ["<<filename<<"]"<<std::endl;
            throw(Error_msg("file failed to open."));
        }
    }

}

void open_fileR(std::fstream& f, const char filename[]) throw(const Error_msg){
    const bool debug = false;   
    //if the file does not exist throw a error msg;
    if(!file_exists(filename)){
        if(debug)
            std::cout << "file open(R) failed" << filename << " does not exist\n";
        throw(Error_msg("file: " + std::string(filename) +" does not exist"));
    }
    else{
        f.open(filename, std::ios::in | std::fstream::binary);
        if(f.fail()){
            std::cout<<"file open (R) failed. ["<<filename<<"]"<<std::endl;
            throw(Error_msg("file failed to open."));
        }
    }
}

void open_fileW(std::fstream& f, const char filename[]) throw(const Error_msg){
    bool debug = false;
    f.open (filename,
            std::fstream::out| std::fstream::binary );
    if (f.fail()){
        if(debug)
            std::cout<<"file open(W) failed: "<<filename<<std::endl;
        throw(Error_msg("file failed to open."));
    }

}
